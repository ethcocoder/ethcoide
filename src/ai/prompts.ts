...
- Focus on the "what" and "why", not just the "how"`;
   ...
   */
  static chat(message: string, context: PromptContext, conversationHistory?: Array<{role: string, content: string}>): string {
    const projectInfo = context.projectContext ? `
...
- Consider the broader context and requirements`;
  ...
  static withProjectContext(basePrompt: string, context: PromptContext): string {
    if (!context.projectContext) return basePrompt;
