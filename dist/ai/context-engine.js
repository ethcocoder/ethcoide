 ...
 */
class ContextEngine {
    constructor(fileService, projectService, config = {}) {
        this.cache = new Map();
        this.metrics = {
            hits: 0,
            misses: 0,
     ...
     */
    async collectContext(currentFilePath, selectedText, cursorLine) {
        ...
        try {
            ...
            if (contextFiles.length < this.config.maxFiles) {
                ...
                for (const fileInfo of relatedFiles) {
                    ...
                    const relatedFile = await this.loadContextFile(path.join(project.rootPath, fileInfo.path), project.rootPath);
     ...
     */
    async loadContextFile(filePath, rootPath) {
        try {
     ...
     */
    async findImports(contextFile, rootPath) {
        const imports = [];
     ...
     */
    async resolveImportPath(importPath, currentFilePath, rootPath) {
        try {
     ...
     */
    async findRelatedFiles(currentFilePath, project) {
     ...
     */
    calculateFileRelevanceScore(file, currentFileName, currentDir) {
        let score = 0;
     ...
     */
    generateContextSummary(contextFiles, truncated) {
     ...
     */
    async saveCacheEntryToDisk(entry) {
        const project = this.projectService.getCurrentProject();
        if (!project)
            return;
        const cacheDir = path.join(project.rootPath, this.config.cacheDirectory);
     ...
     */
    async removeCacheEntryFromDisk(key) {
        const project = this.projectService.getCurrentProject();
        if (!project)
            return;
        const cacheDir = path.join(project.rootPath, this.config.cacheDirectory);
     ...
     */
    generateCacheKey(filePath, lastModified, fileSize) {
     ...
     */
    async getCachedSummary(filePath) {
        if (!this.config.enableCaching)
            return null;
        try {
     ...
     * Cache file summary
     */
    async cacheSummary(filePath, summary, estimatedTokens) {
        if (!this.config.enableCaching)
            return;
        try {
            const stats = await this.fileService.stat(filePath);
